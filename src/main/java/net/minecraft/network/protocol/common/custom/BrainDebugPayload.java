...
public record BrainDebugPayload(BrainDebugPayload.BrainDump brainDump) implements CustomPacketPayload {
   ...
   public BrainDebugPayload(FriendlyByteBuf p_299558_) {
   ...
   public static record BrainDump(UUID uuid, int id, String name, String profession, int xp, float health, float maxHealth, Vec3 pos, String inventory, @Nullable Path path, boolean wantsGolem, int angerLevel, List<String> activities, List<String> behaviors, List<String> memories, List<String> gossips, Set<BlockPos> pois, Set<BlockPos> potentialPois) {
      public BrainDump(FriendlyByteBuf p_298042_) {
         this(p_298042_.readUUID(), p_298042_.readInt(), p_298042_.readUtf(), p_298042_.readUtf(), p_298042_.readInt(), p_298042_.readFloat(), p_298042_.readFloat(), p_298042_.readVec3(), p_298042_.readUtf(), p_298042_.readNullable(Path::createFromStream), p_298042_.readBoolean(), p_298042_.readInt(), p_298042_.readList(FriendlyByteBuf::readUtf), p_298042_.readList(FriendlyByteBuf::readUtf), p_298042_.readList(FriendlyByteBuf::readUtf), p_298042_.readList(FriendlyByteBuf::readUtf), p_298042_.readCollection(HashSet::new, FriendlyByteBuf::readBlockPos), p_298042_.readCollection(HashSet::new, FriendlyByteBuf::readBlockPos));
      }
      ...
      public void write(FriendlyByteBuf p_299431_) {
         ...
         p_299431_.writeNullable(this.path, (p_297936_, p_301045_) -> {
         ...
         p_299431_.writeCollection(this.pois, FriendlyByteBuf::writeBlockPos);
         p_299431_.writeCollection(this.potentialPois, FriendlyByteBuf::writeBlockPos);
      }
