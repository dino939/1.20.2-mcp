...
public record LocationCheck(Optional<LocationPredicate> predicate, BlockPos offset) implements LootItemCondition {
   ...
   public boolean test(LootContext p_81731_) {
      ...
      return vec3 != null && (this.predicate.isEmpty() || this.predicate.get().matches(p_81731_.getLevel(), vec3.x() + (double)this.offset.getX(), vec3.y() + (double)this.offset.getY(), vec3.z() + (double)this.offset.getZ()));
   }
