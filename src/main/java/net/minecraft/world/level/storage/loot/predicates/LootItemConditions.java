...
public class LootItemConditions {
   ...
   private static LootItemConditionType register(String p_81832_, Codec<? extends LootItemCondition> p_300489_) {
      return Registry.register(BuiltInRegistries.LOOT_CONDITION_TYPE, new ResourceLocation(p_81832_), new LootItemConditionType(p_300489_));
   }

   public static <T> Predicate<T> andConditions(List<? extends Predicate<T>> p_300844_) {
      List<Predicate<T>> list = List.copyOf(p_300844_);
      Predicate predicate;
      switch (list.size()) {
         case 0:
            predicate = (p_81840_) -> {
   ...
   public static <T> Predicate<T> orConditions(List<? extends Predicate<T>> p_298478_) {
      List<Predicate<T>> list = List.copyOf(p_298478_);
      Predicate predicate;
      switch (list.size()) {
         case 0:
            predicate = (p_81830_) -> {
