      ...
      if (p_270481_ && path$debugdata != null) {
         for(Node node1 : path$debugdata.closedSet()) {
            if (distanceToCamera(node1.asBlockPos(), p_270187_, p_270252_, p_270371_) <= 80.0F) {
               DebugRenderer.renderFilledBox(p_270399_, p_270359_, (new AABB((double)((float)node1.x + 0.5F - p_270841_ / 2.0F), (double)((float)node1.y + 0.01F), (double)((float)node1.z + 0.5F - p_270841_ / 2.0F), (double)((float)node1.x + 0.5F + p_270841_ / 2.0F), (double)node1.y + 0.1D, (double)((float)node1.z + 0.5F + p_270841_ / 2.0F))).move(-p_270187_, -p_270252_, -p_270371_), 1.0F, 0.8F, 0.8F, 0.5F);
            }
         }
      ...
      if (p_270748_) {
         for(int j = 0; j < p_270189_.getNodeCount(); ++j) {
            ...
            if (distanceToCamera(node2.asBlockPos(), p_270187_, p_270252_, p_270371_) <= 80.0F) {
               DebugRenderer.renderFloatingText(p_270399_, p_270359_, String.valueOf((Object)node2.type), (double)node2.x + 0.5D, (double)node2.y + 0.75D, (double)node2.z + 0.5D, -1, 0.02F, true, 0.0F, true);
               DebugRenderer.renderFloatingText(p_270399_, p_270359_, String.format(Locale.ROOT, "%.2f", node2.costMalus), (double)node2.x + 0.5D, (double)node2.y + 0.25D, (double)node2.z + 0.5D, -1, 0.02F, true, 0.0F, true);
            }
         }
      }
