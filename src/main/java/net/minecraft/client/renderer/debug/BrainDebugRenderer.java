...
public class BrainDebugRenderer implements DebugRenderer.SimpleDebugRenderer {
   ...
   private void renderBrainInfo(PoseStack p_270145_, MultiBufferSource p_270489_, BrainDebugPayload.BrainDump p_299702_, double p_270922_, double p_270468_, double p_270838_) {
      ...
      if (flag) {
         ...
         renderTextOverMob(p_270145_, p_270489_, p_299702_.pos(), i, "health: " + String.format(Locale.ROOT, "%.1f", p_299702_.health()) + " / " + String.format(Locale.ROOT, "%.1f", p_299702_.maxHealth()), j, 0.02F);
         ++i;
      }
   ...
   private Collection<UUID> getTicketHolders(BlockPos p_113285_) {
      return this.brainDumpsPerEntity.values().stream().filter((p_296276_) -> {
         return p_296276_.hasPoi(p_113285_);
   ...
   private Collection<UUID> getPotentialTicketHolders(BlockPos p_113290_) {
      return this.brainDumpsPerEntity.values().stream().filter((p_296280_) -> {
         return p_296280_.hasPotentialPoi(p_113290_);
      }).map(BrainDebugPayload.BrainDump::uuid).collect(Collectors.toSet());
   }
