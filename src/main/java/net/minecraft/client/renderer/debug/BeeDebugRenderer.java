...
public class BeeDebugRenderer implements DebugRenderer.SimpleDebugRenderer {
   ...
   private void renderFlowerInfos(PoseStack p_270578_, MultiBufferSource p_270098_) {
      ...
      map.forEach((p_296266_, p_296267_) -> {
         ...
         int i = 1;
         renderTextOverPos(p_270578_, p_270098_, set.toString(), p_296266_, i++, -256);
         renderTextOverPos(p_270578_, p_270098_, "Flower", p_296266_, i++, -1);
         float f = 0.05F;
         DebugRenderer.renderFilledBox(p_270578_, p_270098_, p_296266_, 0.05F, 0.8F, 0.8F, 0.0F, 0.3F);
      });
   }
   ...
   private void renderPath(PoseStack p_270424_, MultiBufferSource p_270123_, BeeDebugPayload.BeeInfo p_299438_) {
      if (p_299438_.path() != null) {
         PathfindingRenderer.renderPath(p_270424_, p_270123_, p_299438_.path(), 0.5F, false, false, this.getCamera().getPosition().x(), this.getCamera().getPosition().y(), this.getCamera().getPosition().z());
      }
   ...
   private Collection<UUID> getHiveMembers(BlockPos p_113130_) {
      return this.beeInfosPerEntity.values().stream().filter((p_296249_) -> {
         return p_296249_.hasHive(p_113130_);
