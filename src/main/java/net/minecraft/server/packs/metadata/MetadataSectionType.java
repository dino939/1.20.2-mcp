   ...
   static <T> MetadataSectionType<T> fromCodec(final String p_249716_, final Codec<T> p_249525_) {
      return new MetadataSectionType<T>() {
         ...
         public T fromJson(JsonObject p_249450_) {
            return Util.getOrThrow(p_249525_.parse(JsonOps.INSTANCE, p_249450_), JsonParseException::new);
         }

         public JsonObject toJson(T p_250691_) {
            return Util.getOrThrow(p_249525_.encodeStart(JsonOps.INSTANCE, p_250691_), IllegalArgumentException::new).getAsJsonObject();
         }
      };
   }
}