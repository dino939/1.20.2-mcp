...
public class MobEffectIdFix extends DataFix {
   ...
   private static <T> Optional<Dynamic<T>> getAndConvertMobEffectId(Dynamic<T> p_300040_, String p_300405_) {
      return p_300040_.get(p_300405_).asNumber().result().map((p_298913_) -> {
         return ID_MAP.get(p_298913_.intValue());
      }).map(p_300040_::createString);
   }

   private static <T> Dynamic<T> setFieldIfPresent(Dynamic<T> p_298128_, String p_297327_, Optional<Dynamic<T>> p_297377_) {
      return p_297377_.isEmpty() ? p_298128_ : p_298128_.set(p_297327_, p_297377_.get());
   }

   private static <T> Dynamic<T> replaceField(Dynamic<T> p_297407_, String p_298935_, String p_297534_, Optional<Dynamic<T>> p_299072_) {
      return setFieldIfPresent(p_297407_.remove(p_298935_), p_297534_, p_299072_);
   }

   private static <T> Dynamic<T> renameField(Dynamic<T> p_299698_, String p_297388_, String p_300782_) {
      return setFieldIfPresent(p_299698_.remove(p_297388_), p_300782_, p_299698_.get(p_297388_).result());
   }
   ...
   private static <T> Dynamic<T> updateMobEffectIdField(Dynamic<T> p_299189_, String p_301147_, Dynamic<T> p_297288_, String p_297619_) {
      ...
      return replaceField(p_297288_, p_301147_, p_297619_, optional);
   }
   ...
   private static <T> Dynamic<T> updateMobEffectInstanceList(Dynamic<T> p_298694_, String p_298177_, String p_300921_) {
      Optional<Dynamic<T>> optional = p_298694_.get(p_298177_).asStreamOpt().result().map((p_297707_) -> {
         return p_298694_.createList(p_297707_.map(MobEffectIdFix::updateMobEffectInstance));
      });
      return replaceField(p_298694_, p_298177_, p_300921_, optional);
   }
   ...
   private static <T> Dynamic<T> updateSuspiciousStewEntry(Dynamic<T> p_299220_, Dynamic<T> p_300010_) {
      ...
      return replaceField(p_300010_, "EffectDuration", "duration", optional);
   }
   ...
   private Typed<?> updateNamedChoice(Typed<?> p_299605_, DSL.TypeReference p_299152_, String p_300042_, Function<Dynamic<?>, Dynamic<?>> p_300498_) {
      ...
      return p_299605_.updateTyped(DSL.namedChoice(p_300042_, type), type1, (p_299360_) -> {
         return p_299360_.update(DSL.remainderFinder(), p_300498_);
      });
   }
