...
public record TagPredicate<T>(TagKey<T> tag, boolean expected) {
   public static <T> Codec<TagPredicate<T>> codec(ResourceKey<? extends Registry<T>> p_301303_) {
      return RecordCodecBuilder.create((p_299938_) -> {
         return p_299938_.group(TagKey.codec(p_301303_).fieldOf("id").forGetter(TagPredicate::tag), Codec.BOOL.fieldOf("expected").forGetter(TagPredicate::expected)).apply(p_299938_, TagPredicate::new);
      });
   }
